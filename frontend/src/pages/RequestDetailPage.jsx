...
const RequestDetailPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const activeUsers = (usersResponse?.users || []).filter((candidate) => candidate.role === 'volunteer');
  ...
  const backendStatus = request?.backendStatus || request?.status;
  const isAssignableStatus = backendStatus === 'pending';
  const isAwaitingVerification = backendStatus === 'pending-verification';
  const hasAssignee = !!(request?.assignedTo || request?.assigneeName);
  ...
  const handleAssignRequest = async () => {
    ...
    if (!isAssignableStatus) {
      setActionError('This request cannot be assigned until it is in pending status.');
      return;
    }
    ...
    try {
      const updated = await api.assignResources(id, [], assignee.id, {
      ...
      setRequest((current) => ({
        ...current,
        ...updated,
        assignedTo: updated.assignedTo || assignee.id,
        assigneeName: updated.assigneeName || assignee.fullName,
  ...
  const handleAssignToSelf = async () => {
    ...
    if (user.role !== 'volunteer') {
      setActionError('Assign To Me is only available for volunteer accounts.');
      return;
    }
    ...
    setActionLoading(true);
    try {
      const updated = await api.assignResources(id, [], selfCandidate.id, {
        name: selfCandidate.fullName,
        email: selfCandidate.email,
      });
      setRequest((current) => ({
        ...current,
        ...updated,
        assignedTo: updated.assignedTo || selfCandidate.id,
        assigneeName: updated.assigneeName || selfCandidate.fullName,
        status: updated.status || 'assigned',
        backendStatus: updated.backendStatus || 'assigned',
      }));
      await refreshNotificationsFromServer(user);
  ...
  const handleStartTask = async () => {
    ...
    if (!hasAssignee) {
      setActionError('Assign a worker before marking this task as active.');
      return;
    }
    ...
    try {
      ...
      setActionError(error?.message || 'Failed to start this task. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
  ...
  const handleVerifyRequest = async () => {
    if (!canAssignCurrentRequest) {
      setActionError('Your role is not allowed to verify requests.');
      return;
    }

    if (!isAwaitingVerification) {
      setActionError('This request does not need verification.');
      return;
    }
    ...
    try {
      ...
      setActionError(error?.message || 'Failed to verify this request. Please try again.');
    } finally {
      setActionLoading(false);
    }
  };
    ...
    <div className="request-detail-page">
      ...
      <div className="request-detail-grid">
        ...
        <section className="request-detail-card request-detail-full-width">
          ...
          <div className="request-detail-actions-grid">
            <div>
              ...
              <button
                ...
                disabled={actionLoading || !canAssignCurrentRequest || !isAssignableStatus}
                title={!isAssignableStatus ? 'Request must be verified to Pending before assignment.' : ''}
              >
                Assign Request
              </button>
              <button
                ...
                disabled={actionLoading || !canAssignCurrentRequest || !isAssignableStatus}
                title={!isAssignableStatus ? 'Request must be verified to Pending before self-assignment.' : ''}
              >
                Assign To Me
              </button>
              {canAssignCurrentRequest && isAwaitingVerification && (
                <button
                  type="button"
                  className="request-detail-btn"
              ...
              {canAssignCurrentRequest && !isAssignableStatus && (
                <p className="request-detail-description">This request is awaiting verification and cannot be assigned yet.</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="request-detail-action-row">
                <button
                  ...
                  disabled={actionLoading || !canUpdateCurrentRequest || !hasAssignee || request.status === 'in-progress' || request.status === 'completed'}
                  title={!hasAssignee ? 'Assign a worker before starting this task.' : ''}
                >
                  Mark Active
                </button>
                <button
          ...
          <dl className="request-detail-list">
            <div><dt>Backend Status</dt><dd>{backendStatus}</dd></div>
